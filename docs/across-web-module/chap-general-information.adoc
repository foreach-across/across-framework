 ...
 * adding a <<view-elements,programmatic model for creating UI components>> from code
...
The across-web artifact also contains `<<across-application,@AcrossApplication>>`, a replacement for `@SpringBootApplication` when working with an AcrossContext.
