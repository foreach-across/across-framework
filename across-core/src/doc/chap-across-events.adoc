...
You can force an event handler order by assigning an `@Order` annotation on either the bean that declares the handler method, or on the handler method itself.

The default ordering should be sufficient for most use cases: you can relly on event handlers from previous modules having been executed.
