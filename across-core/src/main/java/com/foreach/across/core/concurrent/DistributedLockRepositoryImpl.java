...
public class DistributedLockRepositoryImpl implements DistributedLockRepository
{
	...
	public DistributedLockRepositoryImpl( DataSource dataSource ) {
		jdbcTemplate = new JdbcTemplate( dataSource );
	}

	@Override
	public Lock getLock( String key ) {
		return getSharedLock( buildThreadUniqueOwner(), key );
	}

	@Override
	public Lock getSharedLock( String owner, String key ) {
		Assert.isTrue( StringUtils.isNotBlank( key ) );
		Assert.isTrue( StringUtils.isNotBlank( owner ) );
		return new DistributedLock( this, owner, key );
	}

	private String buildThreadUniqueOwner() {
	...
	public boolean acquire( DistributedLock lock ) {
		...
		try {
			...
			LOG.trace( "Lock {} is held by {} since {} ms", lock.getLockId(), lockInfo.getOwnerId(), (timestamp - lockInfo.getCreated()) );
		}
