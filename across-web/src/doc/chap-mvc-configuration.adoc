...
When building the request mapping info, AcrossWebModule will create beans of every `CustomRequestCondition` and will supply them with the `AnnotatedTypeMetadata` of the handler method or handler type they were configured on.
...
private static class ReferrerRequestCondition extends AbstractCustomRequestCondition<ReferrerRequestCondition>
{
    ...
    public void setAnnotatedTypeMetadata( AnnotatedTypeMetadata metadata ) {
        referrers = (String[]) metadata.getAnnotationAttributes(
                        ReferrerMapping.class.getName()
                    ).get( "value" );
    }
