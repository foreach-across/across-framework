...
public class AcrossWebConfig extends WebMvcConfigurerAdapter
{
	...
	public void addResourceHandlers( ResourceHandlerRegistry registry ) {
		registry.addResourceHandler( acrossWebModule.getViewsResourcePath() + "/css/**" ).addResourceLocations(
				"classpath:/views/css/" );
		...
		if ( acrossWebModule.isDevelopmentMode() ) {
			...
			for ( Map.Entry<String, String> views : acrossWebModule.getDevelopmentViews().entrySet() ) {
				String url = acrossWebModule.getViewsResourcePath() + "/css/" + views.getKey() + "/**";
				String physical = new File( views.getValue(), "css/" + views.getKey() ).toURI().toString();
